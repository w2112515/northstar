...
export const SAMPLE_STARTED = "2026-06-30T09:30:00-04:00";

export const sampleVoyage = (): VoyageConfig => ({
  onboarded: true,
  startingCapital: 100_000,
  goalMode: "amount",
  targetAmount: 110_000,
  monthlyIncome: 800,
  deadlineMonths: 12,
...
export const freshVoyage = (partial: Partial<VoyageConfig>): VoyageConfig => ({
  onboarded: true,
  startingCapital: 100_000,
  goalMode: "amount",
  targetAmount: 110_000,
...
export const sampleCash = 25_362;

export const sampleOrders = (): QueuedOrder[] => [
  {
    id: "ord-spy",
    symbol: "SPY",
    humanName: "SPY",
...
export const sampleLog = (): CaptainsLog => ({
  sentences: [
...
export const sampleJournal = (): JournalEvent[] => [
  {
    id: "j-sys-1",
    ...
    aiNarrated: true,
    refs: ["pass-0829-c"],
    raw: { equity: 102480, todayPct: 1.2, odds: 0.68 },
  },
  {
    id: "j-ord-2",
    ...
    raw: { symbol: "SPY", qty: 8, limit: 643.5, riskPct: 0.0037 },
  },
  {
    id: "j-pr-1",
    ...
    aiNarrated: false,
    refs: ["pr-nvda", "gate"],
    raw: { symbol: "NVDA", qty: 12, riskPct: 0.011, cap: 0.01 },
  },
  {
    id: "j-ver-1",
    ...
    raw: { symbol: "TSLA", attempts: 3, riskPct: 0.021, cap: 0.01 },
  },
  {
    id: "j-deb-1",
  ...
  },
  {
    ...
    aiNarrated: false,
    refs: ["pos-spy"],
    raw: { symbol: "SPY", qty: 8, price: 643.1 },
  },
  {
    id: "j-fc-1",
    ...
    aiNarrated: true,
    refs: ["timesfm-spy"],
    raw: { symbol: "SPY", mid: 0.009, lo: -0.014, hi: 0.026 },
  },
  {
    id: "j-scout-1",
  ...
  },
  {
    ...
    aiNarrated: false,
    refs: ["pos-amzn"],
    raw: { symbol: "AMZN", qty: 12, price: 197.4 },
  },
  {
    id: "j-pnl-1",
  ...
  },
  {
    ...
    aiNarrated: false,
    refs: ["pass-0828-b"],
    raw: { pass: "14b", timingsMs: [80, 12, 40, 210, 8, 1400, 6] },
  },
  {
    id: "j-exp-1",
    ...
    aiNarrated: false,
    refs: ["exp-friday-v2"],
    raw: { oosSharpe: 0.41, result: "failed" },
  },
  {
    id: "j-sys-0",
    ...
    aiNarrated: false,
    refs: ["voyage"],
    raw: { startingCapital: 100000, target: 110000, months: 12 },
  },
];

export const firstDayJournal = (cfg: VoyageConfig): JournalEvent[] => [
  {
    id: "j-start",
    ts: cfg.startedAt,
    kind: "system",
    title: "Voyage started",
    body: `Paper book ${cfg.startingCapital.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 })}. Destination ${
      cfg.goalMode === "amount"
        ? `${cfg.targetAmount.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 })} in ${cfg.deadlineMonths} months`
        : `${cfg.monthlyIncome.toLocaleString("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 })} / month`
    }. Temperament ${cfg.temperament}. Nothing has sailed yet.`,
    aiNarrated: false,
    refs: ["voyage"],
    raw: { ...cfg },
  },
...
export const firstDayLog = (): CaptainsLog => ({
  sentences: [
    "Fleet is assembled. Nothing has sailed yet.",
    "We wait for the open. The gate is awake; the book is cash.",
  ],
  aiNarrated: false,
  ts: new Date().toISOString(),
});

export const sampleStrategies = (): StrategyDef[] => [
  {
    id: "st-core",
    name: "Core Trend",
...
export const sampleInstances = (): StrategyInstance[] => [
  {
    id: "in-core",
    strategyId: "st-core",
    family: "trend",
...
export const sampleScouts = (): ScoutCandidate[] => [
  { id: "sc-avgo", symbol: "AVGO", score: 78, why: "Breadth held, relative strength vs SOX, IV not stretched.", flavors: ["momentum", "quality"] },
  { id: "sc-lly", symbol: "LLY", score: 71, why: "Quiet grind, low residual vs XLV, weather-floor friendly.", flavors: ["low-vol", "quality"] },
  { id: "sc-crwd", symbol: "CRWD", score: 64, why: "Passed radar, lost the debate on valuation. Watching, not chasing.", flavors: ["momentum"] },
  { id: "sc-cost", symbol: "COST", score: 61, why: "Defensive drift in a calm-up tape. Slow, not exciting.", flavors: ["quality", "low-vol"] },
  { id: "sc-jpm", symbol: "JPM", score: 58, why: "Yield curve analog is friendly. Size would be tiny.", flavors: ["value"] },
];

export const sampleOptionWatch = (): OptionWatch[] => [
  { id: "ow-1", humanName: "NVDA 19 Sep 180 C", yield: 0.084, dte: 20, iv: 0.41, note: "Already on the book. Trim queued." },
  { id: "ow-2", humanName: "SPY 18 Sep 650 C", yield: 0.031, dte: 19, iv: 0.14, note: "Too cheap. Gate would say no on edge." },
  { id: "ow-3", humanName: "AAPL 19 Sep 245 C", yield: 0.052, dte: 20, iv: 0.22, note: "Borderline. Weather floor says wait." },
];

export const samplePromotions = (): PromotionCandidate[] => [
  {
    id: "pm-1",
...
export const sampleExperiments = (): ExperimentRow[] => [
  { id: "ex-1", name: "Weather Floor v2.1", family: "overlay", result: "promoted", oosSharpe: 1.67, note: "Champion. Live since June.", ts: "2026-06-28T12:00:00-04:00" },
  { id: "ex-2", name: "Drift Harvest v3.0", family: "factors", result: "promoted", oosSharpe: 1.31, note: "Replaced v2.4 after IC decay.", ts: "2026-07-12T12:00:00-04:00" },
  { id: "ex-3", name: "Friday-hedge v2", family: "hedge", result: "failed", oosSharpe: 0.41, note: "Did not hold in 2022 analog.", ts: "2026-08-27T09:12:00-04:00" },
  { id: "ex-4", name: "Gap fade v1", family: "tactical", result: "failed", oosSharpe: 0.22, note: "Spread tax. Archived.", ts: "2026-08-04T16:40:00-04:00" },
  { id: "ex-5", name: "Night Watch v0.4", family: "hedge", result: "running", oosSharpe: 0.88, note: "Shipyard. Not live.", ts: "2026-08-20T11:00:00-04:00" },
  { id: "ex-6", name: "Debate Overlay v1.2", family: "overlay", result: "promoted", oosSharpe: 1.05, note: "Low turnover, high trust.", ts: "2026-07-30T10:00:00-04:00" },
];

export const sampleFactors = (): FactorRow[] => [
  { id: "fa-1", name: "12-1 momentum", ic: 0.046, deflatedIc: 0.031, horizon: "21d", decay: "fresh", admitted: true },
  { id: "fa-2", name: "5d reversal", ic: -0.022, deflatedIc: -0.009, horizon: "5d", decay: "aging", admitted: true },
  { id: "fa-3", name: "Earnings surprise", ic: 0.061, deflatedIc: 0.038, horizon: "10d", decay: "fresh", admitted: true },
  { id: "fa-4", name: "IV residual", ic: 0.028, deflatedIc: 0.011, horizon: "10d", decay: "aging", admitted: true },
  { id: "fa-5", name: "Breadth thrust", ic: 0.019, deflatedIc: 0.004, horizon: "5d", decay: "decayed", admitted: false },
  { id: "fa-6", name: "Quality minus junk", ic: 0.033, deflatedIc: 0.021, horizon: "63d", decay: "fresh", admitted: true },
  { id: "fa-7", name: "Skip-week seasonality", ic: 0.072, deflatedIc: 0.014, horizon: "5d", decay: "fresh", admitted: false, pending: true },
];

export const sampleAdvice = (): HelmAdvice => ({
  id: "adv-1",
...
export const sampleWatchlist = (): WatchItem[] => [
  { symbol: "SPY", last: 644.2, change: 0.007, group: "holdings" },
  { symbol: "NVDA", last: 178.4, change: 0.018, group: "holdings" },
  { symbol: "AAPL", last: 241.1, change: 0.006, group: "holdings" },
  { symbol: "MSFT", last: 428.6, change: 0.004, group: "holdings" },
  { symbol: "AVGO", last: 181.2, change: 0.012, group: "scout" },
  { symbol: "LLY", last: 794.5, change: 0.003, group: "scout" },
  { symbol: "CRWD", last: 412.8, change: -0.011, group: "scout" },
  { symbol: "QQQ", last: 512.8, change: 0.008, group: "core" },
  { symbol: "IWM", last: 228.4, change: -0.002, group: "core" },
  { symbol: "TLT", last: 91.6, change: -0.004, group: "core" },
];
...
export function buildCandles(symbol: string, last: number, n = 42): Candle[] {
  const seed =
    symbol.split("").reduce((s, c) => s + c.charCodeAt(0), 0) * 997 +
    Math.round(last * 10);
  const rand = mulberry32(seed);
  const out: Candle[] = [];
  let p = last * 0.93;
  const start = new Date("2026-07-01T16:00:00-04:00");
  let day = 0;
  while (out.length < n) {
    ...
    const shock = (rand() - 0.48) * last * 0.012;
    const o = p;
    const c = Math.max(0.5, o + drift + shock);
    const h = Math.max(o, c) + rand() * last * 0.006;
    const l = Math.min(o, c) - rand() * last * 0.006;
    out.push({
      t: d.toISOString(),
      o: round2(o),
      h: round2(h),
...
export function buildForecast(last: number): ForecastPoint[] {
  const out: ForecastPoint[] = [];
  const start = new Date("2026-08-31T16:00:00-04:00");
  let mid = last;
  for (let i = 0; i < 5; i++) {
    const d = new Date(start);
    d.setDate(start.getDate() + i);
    mid = mid * 1.0018;
    const w = last * (0.006 + i * 0.004);
    out.push({
      t: d.toISOString(),
      mid: round2(mid),
      lo: round2(mid - w),
...
export const sampleSpark = [
  100000, 100240, 99880, 100410, 100905, 101120, 100760, 101340, 101580, 101210,
  101770, 102040, 101890, 102260, 102480,
];
