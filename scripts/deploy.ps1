...
gcloud run deploy northstar-api `
    ...
    --allow-unauthenticated --memory 1Gi --cpu 1 --min-instances 1 --max-instances 2 `
    --set-env-vars "ALPACA_API_KEY=$($envVars.ALPACA_API_KEY),ALPACA_SECRET_KEY=$($envVars.ALPACA_SECRET_KEY),ALPACA_PAPER=true,ACCOUNT_ROLE=$($envVars.ACCOUNT_ROLE),GOOGLE_API_KEY=$($envVars.GOOGLE_API_KEY),JOURNAL_STORE=firestore,GOOGLE_CLOUD_PROJECT=$ProjectId"
