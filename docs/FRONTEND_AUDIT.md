...
> **v4.1 对抗审计轮（2026-08-31 深夜）**：v4（Grok 忠实实现）落地后，双设计师代理对当前 runtime 发起对抗审计（A=视觉保真/工艺 23 项，B=交互结构/体验 31 项；完整报告与取证截图在 `artifacts/ui-attack/`，gitignored，可用 `scripts/capture-ui.mjs` 再生）。两代理独立命中同一批承重缺陷，按共享根因收敛为六组并全部修复：
> ① **CSS 承重层**（P0/P1）：focus 双环写在 `@layer base` 的 box-shadow 上，被 utilities 层 `shadow-border`/`shadow-tone-*` 覆盖——向导全部输入框、金色 CTA、Kill switch 键盘聚焦零指示 → 改 outline 双环（不与 shadow 竞争）；shell 根节点 `overflow-x-hidden` 制造滚动容器语义，全站 sticky（顶栏/KPI/Journal 日期头）从未吸附 → 改 `overflow-x-clip` + Journal 吸附偏移校准至实测 header 高度。
> ② **图表退化态**（P1）：TrajectoryHero 概率标尺 above 档标签从零高锚点向上溢出、压进图例（`h-16`→留白重排）；红路径目标线超出数据域被 recharts 静默丢弃（`ifOverflow="extendDomain"`），图表与 "0% odds" 判词矛盾；X 轴月刻度跳号改显式逐月；onboarding 预览图例的幽灵 "actual equity" 项条件化。
> ③ **信任反馈**（P1）：Overview Controls 三个全局动作（Autopilot/Run one pass/**Kill switch**）静默吞错 → 失败条幅化（与 Research 同款）；Close 持仓结果是 10px 灰脚注 10 秒自毁 → 语义分色 + `role="status"` + 不自毁；审批卡裸红数字加 "Worst case" 标签 + 补渲染 `thesis_human`。
> ④ **语义色锁**（P2 质量线）：odds 回 gold（契约裁决：默认 gold，红路径档 coral，amber 永不用于 odds——已写入 DESIGN.md v4.1）；TimesFM 中线 gold→signal（预测不得穿事实色）；amber 收口只剩"等待人类"（medium risk/regime stressed/queued→mist，连接类错误横幅→coral）；gold 通胀清理（序号/t-stat/权重/等待文案/pass running 徽章→各归其位）；champion 卡 ring teal→gold；signal 实底按钮 text-ink(2.77:1)→text-void(5.79:1)；eventTone pnl 按正负取色、digest/debate 徽章去金。
> ⑤ **文案黑名单**（P1）：前端自产 "waiting on you(r)" ×3 改写；后端 10 处源头（journal human/AI prompt/目录文案/weather/report："Voyage plan activated"、"fleet's navigator"、"took the helm"、"Captain's log"、"Helm advisor"、"sails/sailing" 等）全部改词；新增 `scripts/check-copy.mjs` 防线并挂进 `npm run lint`（扫 `apps/web/src` 全部 + `apps/api` 人读文案，词表含 voyage/fleet/helm/sail/captain/dock/on board/waiting on you；`captain` 作为 journal payload 键名与函数名白名单放行——改名会断旧数据）。已落库的旧 journal 事件按 append-only 原则不追改（诚实声明）。
> ⑥ **签名视觉与工艺**（P2）：GoalOrbit 对称半椭圆→Grok 单调上升 bezier，progress<3% 时出发点标签避让（新用户第一屏叠印），15 处硬编码 hex→theme 单源；空闲态 hero 补静态星场纹理（方向级问题 Q1 留档：常驻缩小轨道待契约 owner 裁决）；导航/tabs 11px 大写→13-14px 句首大写（按 Grok 基准回摆，kicker 体系保持 11px 大写）；Tabs ARIA 空壳降级为诚实 `aria-pressed`；recharts `accessibilityLayer` 关闭（role="img" 内的可聚焦 application 冲突）；移动触控目标（Close/筛选 chips/fan 符号/ticker 输入）补 44px 档；法务/承诺行 `mist/60`(3.9:1) 提到达标浓度；向导 Horizon 边界校验 + 清空回弹修复 + 退出动线（logo 链接 + Keep my current plan）；kill 态与 pass 态 hero kicker 区分 + Approve 禁用解释 + 条幅补 Release 路径；表格 `scope`、温度计 radiogroup、Needs you 空态面板、feed 两行截断 + Journal 链接、archived 实例过滤、jargon 首现解释（OOS/IC/option/short）。
> **记账不修（转产品债）**：红路径 alternatives 结构化可点（需后端字段设计，过渡为 Adjust 聚焦输入）；kill 态 hero 完整 DESTINATION 框架；pipeline 卫星 note 完整内联（先补 aria-label）；digest payload 键名 `captain`（前端读取依赖，改名会断旧事件）；Journal 搜索仅限已加载条目（空态已加提示）；Experiment payload 无 spec 名 → 候选卡暂以短 id 后缀（`#a1b2`）区分同族卡，后端补字段后换正名；策略目录名 "Shipyard Rotation (DSL)" 属航海系边缘词（不在契约词表内，改名是产品命名决策，留给 owner）。三个实时状态（pass-running/kill/approved）本轮仍未实拍（禁触发），代码级修复待真机复验。
>
> **v4.1 收口（同夜验证）**：`npm run lint`（eslint + check-copy）绿；`next build` 绿（复验中补一处 TS 错误：`Button` 未透传 `title`）；后端 `uv run pytest` **210 全过**（文案改词零破坏，测试断言的是事实字段而非叙事前缀）；五页 SSR 200 + 五旧路由 307；14 张复截图逐条核销（`artifacts/ui-attack/`，改前基线保留在 `artifacts/ui-attack-before/`）。复截图揪出一处新 bug 并已修：黄档（stretch）计划的 honest alternatives 块错穿 coral "Red path" 皮——coral 只属红档，黄档降为中性 panel-inset "Ways to raise the odds"。**残余（数据而非代码）**：已落库的 journal 事件与 AI 假设文本仍带旧词（"Voyage plan activated"、"As the fleet's navigator…"），生成源已全部改词，下次 nightly/事件自然替换，按 append-only 不追改历史。
>
