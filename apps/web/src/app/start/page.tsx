"use client";

/** /start - the one dark room. The promise before the proof: pick a
 *  destination under the night sky, get the honest odds, then step into the
 *  daylight ledger where everything is on record. Four steps, same honest
 *  math as always. */

import { useMemo, useState } from "react";
import { useRouter } from "next/navigation";
import {
  Area,
  AreaChart,
  CartesianGrid,
  Line,
  ResponsiveContainer,
  Tooltip,
  XAxis,
  YAxis,
} from "recharts";
import { apiPost, fmtPct, fmtUsd } from "@/lib/api";
import { AXIS_TICK_DARK, CHART_DARK, RECHARTS_TOOLTIP_DARK } from "@/lib/theme";
import { Button, Stamp } from "@/components/ui";
import { GoalOrbit } from "@/components/orbit";

type Preview = {
  goal: Record<string, unknown>;
  plan: {
    feasibility: "green" | "yellow" | "red";
    probability: number;
    required_annual_return: number;
    max_drawdown_est: number;
    allocations: { strategy_id: string; weight: number; why: string }[];
    guardrails: Record<string, number>;
    baseline_note: string;
    honest_alternatives: string[];
  };
  bands: { p10: number[]; p50: number[]; p90: number[] };
  months: number;
  target_amount: number;
  data_note: string;
};

const RISK_QUESTIONS = [
  {
    q: "Your account drops 10% in a rough month. What's your honest reaction?",
    opts: [
      { label: "Stop everything, I can't sleep", score: 0 },
      { label: "Uncomfortable, but I'd trim risk and continue", score: 1 },
      { label: "Normal weather - stay the course", score: 2 },
    ],
  },
  {
    q: "This money is...",
    opts: [
      { label: "Savings I may need within a year", score: 0 },
      { label: "Medium-term money (1-3 years)", score: 1 },
      { label: "Long-term money I won't touch", score: 2 },
    ],
  },
  {
    q: "Which sentence sounds most like you?",
    opts: [
      { label: "Slow and steady beats sorry", score: 0 },
      { label: "Balance growth and safety", score: 1 },
      { label: "I accept bigger swings for bigger upside", score: 2 },
    ],
  },
];

const LEVELS = ["conservative", "balanced", "aggressive"] as const;

const INPUT_CLS =
  "w-full rounded-sm border border-hairline bg-inset px-3 py-2 text-body text-ink transition-colors focus:border-indigo/60";

function StepTitle({ children, sub }: { children: string; sub?: string }) {
  return (
    <div className="mb-5">
      <h2 className="text-page font-semibold tracking-tight text-ink">{children}</h2>
      {sub ? <p className="mt-1 text-body text-ink2">{sub}</p> : null}
    </div>
  );
}

export default function Start() {
  const router = useRouter();
  const [step, setStep] = useState(1);
  const [capital, setCapital] = useState(100000);
  const [mode, setMode] = useState<"target_amount" | "monthly_income">("target_amount");
  const [target, setTarget] = useState(110000);
  const [months, setMonths] = useState(12);
  const [monthly, setMonthly] = useState(800);
  const [answers, setAnswers] = useState<number[]>([1, 1, 1]);
  const [preview, setPreview] = useState<Preview | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const risk = useMemo(() => {
    const total = answers.reduce((a, b) => a + b, 0);
    return LEVELS[total <= 2 ? 0 : total <= 4 ? 1 : 2];
  }, [answers]);

  // Validate before enabling the next step - a nonsense goal produces a
  // nonsense plan, and the preview API should never see one.
  const capitalOk = capital >= 25000;
  const targetOk = mode === "monthly_income" ? monthly > 0 : target > capital;
  const step1Ok = capitalOk && targetOk;

  const goalBody = useMemo(
    () => ({
      mode,
      capital_base: capital,
      target_amount: mode === "target_amount" ? target : null,
      horizon_months: months,
      monthly_target: mode === "monthly_income" ? monthly : null,
      risk_level: risk,
    }),
    [mode, capital, target, months, monthly, risk],
  );

  async function loadPreview() {
    setLoading(true);
    setError("");
    try {
      const p = await apiPost<Preview>("/api/goal/preview", goalBody);
      setPreview(p);
      setStep(3);
    } catch {
      setError("Could not compute the plan - is the API running?");
    } finally {
      setLoading(false);
    }
  }

  async function commit() {
    setLoading(true);
    try {
      await apiPost("/api/goal/commit", goalBody);
      router.push("/");
    } catch {
      setError("Commit failed - try again.");
      setLoading(false);
    }
  }

  const chartData = useMemo(() => {
    if (!preview?.bands?.p50) return [];
    return preview.bands.p50.map((v, i) => ({
      month: i + 1,
      p10: preview.bands.p10[i],
      p50: v,
      p90: preview.bands.p90[i],
    }));
  }, [preview]);

  const feas = preview?.plan.feasibility;
  const feasTone = feas === "green" ? "green" : feas === "yellow" ? "amber" : "red";
  const feasText =
    feas === "green"
      ? "Realistic destination"
      : feas === "yellow"
        ? "Stretch goal - doable, not likely"
        : "Honestly unrealistic on this setup";

  return (
    <div className="start-dark starfield min-h-screen w-full bg-paper text-ink">
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        <div className="mb-8">
          <div className="font-mono text-micro uppercase tracking-[0.2em] text-ink2">
            NorthStar
          </div>
          <h1 className="mt-1 text-display font-semibold tracking-tight">
            Pick a star. We&apos;ll tell you the honest odds.
          </h1>
        </div>

        {/* step indicator: completed steps are clickable waypoints */}
        <div className="mb-8 flex flex-wrap items-center gap-2">
          {[1, 2, 3, 4].map((s) => (
            <div key={s} className="flex items-center gap-2">
              {s < step ? (
                <button
                  onClick={() => setStep(s)}
                  aria-label={`Back to step ${s}`}
                  title="Back to this step"
                  className="grid h-7 w-7 place-items-center rounded-full border border-star bg-star/15 font-mono text-body font-semibold text-star transition-colors hover:bg-star/25"
                >
                  {s}
                </button>
              ) : (
                <span
                  className={`grid h-7 w-7 place-items-center rounded-full border font-mono text-body font-semibold ${
                    step === s ? "border-star bg-star/15 text-star" : "border-hairline text-ink2"
                  }`}
                >
                  {s}
                </span>
              )}
              {s < 4 && <span className="h-px w-8 bg-hairline max-sm:hidden" />}
            </div>
          ))}
          <span className="ml-3 text-body text-ink2 max-sm:ml-0 max-sm:basis-full">
            {["Destination", "Risk temperament", "The honest plan", "Begin"][step - 1]}
          </span>
        </div>

        {step === 1 && (
          <div>
            <StepTitle sub="Tell us where you want to go. We'll translate it into a plan - and tell you the truth about the odds.">
              Step 1 - Pick your destination
            </StepTitle>

            {mode === "target_amount" && target > capital && (
              <GoalOrbit base={capital} target={target} current={capital} className="mb-6" />
            )}

            <label className="mb-1 block font-mono text-micro uppercase tracking-[0.12em] text-ink2">
              Practice capital (paper dollars)
            </label>
            <input
              type="number"
              value={capital}
              min={25000}
              step={5000}
              onChange={(e) => setCapital(Number(e.target.value))}
              className={`${INPUT_CLS} mb-1`}
            />
            {!capitalOk && (
              <p className="mb-2 font-mono text-micro text-amber">Minimum practice capital is $25,000.</p>
            )}
            <div className="mb-4" />

            <div className="mb-4 flex gap-2">
              <Button
                variant={mode === "target_amount" ? "primary" : "ghost"}
                onClick={() => setMode("target_amount")}
              >
                Reach an amount
              </Button>
              <Button
                variant={mode === "monthly_income" ? "primary" : "ghost"}
                onClick={() => setMode("monthly_income")}
              >
                Monthly income
              </Button>
            </div>

            {mode === "target_amount" ? (
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block font-mono text-micro uppercase tracking-[0.12em] text-ink2">
                    Grow it to…
                  </label>
                  <input
                    type="number"
                    value={target}
                    step={1000}
                    onChange={(e) => setTarget(Number(e.target.value))}
                    className={INPUT_CLS}
                  />
                  {!targetOk && (
                    <p className="mt-1 font-mono text-micro text-amber">
                      The target has to be above your capital - growth is the point.
                    </p>
                  )}
                </div>
                <div>
                  <label className="mb-1 block font-mono text-micro uppercase tracking-[0.12em] text-ink2">
                    Within…
                  </label>
                  <select
                    value={months}
                    onChange={(e) => setMonths(Number(e.target.value))}
                    className={INPUT_CLS}
                  >
                    {[6, 12, 18, 24, 36].map((m) => (
                      <option key={m} value={m}>
                        {m} months
                      </option>
                    ))}
                  </select>
                </div>
              </div>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block font-mono text-micro uppercase tracking-[0.12em] text-ink2">
                    Income per month
                  </label>
                  <input
                    type="number"
                    value={monthly}
                    step={100}
                    min={1}
                    onChange={(e) => setMonthly(Number(e.target.value))}
                    className={INPUT_CLS}
                  />
                  {!targetOk && (
                    <p className="mt-1 font-mono text-micro text-amber">
                      Income has to be a positive number.
                    </p>
                  )}
                </div>
                <div>
                  <label className="mb-1 block font-mono text-micro uppercase tracking-[0.12em] text-ink2">
                    Judged over…
                  </label>
                  <select
                    value={months}
                    onChange={(e) => setMonths(Number(e.target.value))}
                    className={INPUT_CLS}
                  >
                    {[6, 12, 24].map((m) => (
                      <option key={m} value={m}>
                        {m} months
                      </option>
                    ))}
                  </select>
                </div>
              </div>
            )}

            <div className="mt-6 flex justify-end">
              <Button onClick={() => setStep(2)} disabled={!step1Ok}>
                Next
              </Button>
            </div>
          </div>
        )}

        {step === 2 && (
          <div>
            <StepTitle sub="Three honest questions. They set your guardrails - how much any single trade may risk and when the system stops itself.">
              Step 2 - Risk temperament
            </StepTitle>
            {RISK_QUESTIONS.map((rq, qi) => (
              <div key={qi} className="mb-4">
                <div className="mb-2 text-body">{rq.q}</div>
                <div className="flex flex-wrap gap-2">
                  {rq.opts.map((o, oi) => (
                    <button
                      key={oi}
                      onClick={() => setAnswers((a) => a.map((v, i) => (i === qi ? o.score : v)))}
                      className={`rounded-sm border px-3 py-1.5 text-body transition-colors ${
                        answers[qi] === o.score
                          ? "border-star bg-star/15 text-star"
                          : "border-hairline text-ink2 hover:text-ink"
                      }`}
                    >
                      {o.label}
                    </button>
                  ))}
                </div>
              </div>
            ))}
            <div className="mb-4 border border-hairline bg-inset p-3 text-body">
              Your temperament:{" "}
              <span className="font-mono font-semibold text-star">{risk}</span>
              <span className="ml-2 text-body text-ink2">
                {risk === "conservative"
                  ? "max 0.5% of the account at risk per trade, autopilot pauses at -5% drawdown"
                  : risk === "balanced"
                    ? "max 1% of the account at risk per trade, autopilot pauses at -8% drawdown"
                    : "max 2% of the account at risk per trade, autopilot pauses at -10% drawdown"}
              </span>
            </div>
            <div className="flex justify-between">
              <Button variant="ghost" onClick={() => setStep(1)}>
                Back
              </Button>
              <Button onClick={loadPreview} disabled={loading || !step1Ok}>
                {loading ? "Computing honest odds…" : "See my plan"}
              </Button>
            </div>
            {error && <p className="mt-3 text-body text-red">{error}</p>}
          </div>
        )}

        {step === 3 && preview && (
          <div className="space-y-6">
            <div>
              <StepTitle sub="Simulated on historical returns. An estimate, never a promise.">
                Step 3 - The honest plan
              </StepTitle>
              <div className="flex flex-wrap items-end justify-between gap-4 border border-hairline bg-raised px-5 py-4">
                <div>
                  <div className="font-mono text-micro uppercase tracking-[0.12em] text-ink2">
                    Estimated odds of reaching
                  </div>
                  <div className="mt-0.5 text-display font-semibold">
                    {fmtUsd(preview.target_amount)}{" "}
                    <span className="text-body font-normal text-ink2">in {preview.months} months</span>
                  </div>
                </div>
                <div className="text-right">
                  <div
                    className={`font-mono text-hero font-bold tabular-nums ${
                      feas === "green" ? "text-green" : feas === "yellow" ? "text-amber" : "text-red"
                    }`}
                  >
                    {fmtPct(preview.plan.probability, 0)}
                  </div>
                  <Stamp tone={feasTone as "green" | "amber" | "red"}>{feasText}</Stamp>
                </div>
              </div>

              <div className="mt-4 h-56 border border-hairline bg-inset px-2 py-2">
                <ResponsiveContainer>
                  <AreaChart data={chartData} margin={{ left: 8, right: 8, top: 8 }}>
                    <CartesianGrid stroke={CHART_DARK.grid} strokeDasharray="3 3" vertical={false} />
                    <XAxis dataKey="month" tick={AXIS_TICK_DARK} tickLine={false} />
                    <YAxis
                      tick={AXIS_TICK_DARK}
                      tickLine={false}
                      axisLine={false}
                      tickFormatter={(v: number) => `$${Math.round(v / 1000)}k`}
                      domain={["auto", "auto"]}
                    />
                    <Tooltip
                      contentStyle={RECHARTS_TOOLTIP_DARK}
                      formatter={(v, name) => [fmtUsd(Number(v ?? 0)), String(name)]}
                      labelFormatter={(l) => `Month ${l}`}
                    />
                    <Area dataKey="p90" name="Lucky path (90th)" stroke="none" fill={CHART_DARK.green} fillOpacity={0.12} />
                    <Area dataKey="p10" name="Rough path (10th)" stroke="none" fill={CHART_DARK.inset} fillOpacity={1} />
                    <Line dataKey="p50" name="Median" stroke={CHART_DARK.star} strokeWidth={2} dot={false} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              <p className="mt-2 font-mono text-micro leading-relaxed text-ink2">
                {preview.data_note}
              </p>
            </div>

            {feas !== "green" && (
              <div className="border-l-2 border-red bg-red/5 px-4 py-3">
                <div className="mb-2 font-mono text-micro font-semibold uppercase tracking-[0.12em] text-red">
                  We won&apos;t pretend - here&apos;s what would actually work
                </div>
                <ul className="space-y-2 text-body">
                  {preview.plan.honest_alternatives.map((a, i) => (
                    <li key={i} className="flex gap-2">
                      <span className="text-red">→</span>
                      <span>{a}</span>
                    </li>
                  ))}
                </ul>
                <div className="mt-3">
                  <Button variant="ghost" onClick={() => setStep(1)}>
                    Adjust my destination
                  </Button>
                </div>
              </div>
            )}

            <div>
              <div className="mb-2 font-mono text-micro uppercase tracking-[0.12em] text-ink2">
                The strategies &amp; the guardrails
              </div>
              <div className="grid gap-3 sm:grid-cols-3">
                {preview.plan.allocations.map((a) => (
                  <div key={a.strategy_id} className="border border-hairline bg-raised p-3">
                    <div className="flex items-center justify-between">
                      <span className="text-body font-medium capitalize">{a.strategy_id.replace("_", " ")}</span>
                      <span className="font-mono text-section font-semibold text-star">
                        {fmtPct(a.weight, 0)}
                      </span>
                    </div>
                    <p className="mt-1 text-body text-ink2">{a.why}</p>
                  </div>
                ))}
              </div>
              <ul className="mt-4 grid gap-1.5 text-body text-ink2 sm:grid-cols-2">
                <li>· One trade can risk at most {fmtPct(preview.plan.guardrails.max_loss_per_trade_pct, 1)} of the account</li>
                <li>· No single company above {fmtPct(preview.plan.guardrails.single_name_concentration, 0)}</li>
                <li>· Autopilot pauses for you at {fmtPct(preview.plan.guardrails.breaker_soft_dd, 0)} drawdown</li>
                <li>· Hard stop at {fmtPct(preview.plan.guardrails.breaker_hard_dd, 0)} - no new trades, full stop</li>
                <li>· Expected worst stretch ~{fmtPct(preview.plan.max_drawdown_est, 0)} (median simulated)</li>
                <li>· Kill switch is always one click away</li>
              </ul>
              {preview.plan.baseline_note && (
                <p className="mt-3 border border-hairline bg-inset p-3 text-body leading-relaxed text-ink2">
                  {preview.plan.baseline_note}
                </p>
              )}
            </div>

            <div className="flex justify-between">
              <Button variant="ghost" onClick={() => setStep(2)}>
                Back
              </Button>
              <Button onClick={() => setStep(4)}>Looks honest - continue</Button>
            </div>
          </div>
        )}

        {step === 4 && preview && (
          <div>
            <StepTitle sub="Paper water first.">
              Step 4 - Begin
            </StepTitle>
            <ul className="space-y-2 text-body leading-relaxed">
              <li>✦ This runs on a <b>practice account</b> - simulated money, live market. No real dollars move.</li>
              <li>✦ The autopilot only trades inside the guardrails you just saw. Anything unusual waits for your tap.</li>
              <li>✦ Every decision - including rejected ones - lands on the Proof page in plain English.</li>
              <li>✦ You can pause, change the plan, or hit the kill switch at any time.</li>
            </ul>
            <div className="mt-6 flex justify-between">
              <Button variant="ghost" onClick={() => setStep(3)}>
                Back
              </Button>
              <Button onClick={commit} disabled={loading}>
                {loading ? "Starting…" : "Start the plan"}
              </Button>
            </div>
            {error && <p className="mt-3 text-body text-red">{error}</p>}
          </div>
        )}

        {/* the commitment happens here, so the disclaimer lives here too */}
        <div className="mt-12 border-t border-hairline pt-4 text-center font-mono text-micro leading-relaxed text-ink2">
          Paper trading only - simulated money, real market data. Probabilities are estimates from
          historical patterns, not promises. Nothing here is investment advice.
        </div>
      </div>
    </div>
  );
}
